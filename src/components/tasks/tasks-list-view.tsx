...
export function TasksListView({
  ...
  const [availableTypes, setAvailableTypes] = useState<{name: string, color?: string}[]>([
  ...
  const [availableStatuses, setAvailableStatuses] = useState<{name: string, color?: string, category?: string}[]>([
  ...
  const getStatusColor = (status: string) => {
    ...
    const statusItem = availableStatuses.find((s: {name: string, color?: string}) => s.name === normalizedStatus)
    if (statusItem?.color) {
  ...
  const getTaskTypeColor = (type: string) => {
    ...
    const typeItem = availableTypes.find((t: {name: string, color?: string}) => t.name === type)
    if (typeItem?.color) {
      // Convert hex color to tailwind-like classes
      const colorMap: Record<string, string> = {
  ...
  const normalizeStatusFromAPI = (apiStatus: string): "todo" | "in-progress" | "done" => {
        ...
        setTasks(prevTasks =>
          prevTasks.map(task =>
            task.id === taskId
              ? {
                  ...task,
                  assignees: task.assignees ? [...task.assignees, userData] : [userData],
  ...
  const handleStatusChange = async (taskId: string, status: string) => {
    ...
    try {
      ...
      console.log('📤 Normalized status for API:', normalizedStatus)
      ...
      setTasks(prevTasks => {
        const updatedTasks = prevTasks.map(task => {
          if (task.id === taskId) {
            ...
            return { 
              ...task, 
              status: result.status, // Use the status returned from API
  ...
  const handlePriorityChange = async (taskId: string, priority: string) => {
    ...
    try {
      ...
      setTasks(prevTasks =>
        prevTasks.map(task =>
          task.id === taskId
          ? { ...task, priority: priority as "low" | "medium" | "high" | "critical" }
          : task
        )
      )
            ...
            <div className="flex items-center gap-4">
              ...
              {selectedTasks.length > 0 && (
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="outline" size="sm" className="h-8" onClick={handleBulkEdit}>
                ...
                >
                  ...
                  <td className="px-4 py-4">
                    {task.type ? (
                      <Badge
                        variant="outline"
                        className={`text-xs font-medium ${getTaskTypeColor(task.type)}`}
                      >
                        {formatTaskType(task.type)}
                      </Badge>
                    ) : (
                      <span className="text-sm text-gray-400 dark:text-gray-500">Task</span>
                    )}
                  </td>
                  ...
                  <td className="px-4 py-4">
                    <Select 
                      value={task.status} 
                      onValueChange={(value) => {
                        console.log('🎯 Dropdown changed:', { taskId: task.id, oldStatus: task.status, newStatus: value })
                    ...
                    >
                      ...
                      <SelectContent className="z-50">
                        {availableStatuses.map((status) => {
                          ...
                          console.log('🎨 Rendering status option:', { apiName: status.name, uiValue, displayText: getStatusDisplayText(uiValue) })
                          return (
                            <SelectItem 
                              key={status.name} 
                              value={uiValue}
                  ...
                  <td className="px-4 py-4">
                    <Select value={task.priority} onValueChange={(value) => handlePriorityChange(task.id, value)}>
                      <SelectTrigger className="w-auto h-8 p-1 border-0 bg-transparent">
                        <Badge 
                          variant="outline" 
                          className={`text-xs font-medium cursor-pointer ${getPriorityColor(task.priority)}`}
                        >
                        ...
                        {task.assignees.slice(0, 2).map((assignee, index) => (
                          <div key={assignee.id} className="flex items-center gap-2">
                            ...
                            <span className="text-sm text-gray-900 dark:text-gray-100">
                              {assignee.name}
                              {index === 0 && task.assignees && task.assignees.length > 2 ? ` +${task.assignees.length - 2}` : ''}
                            </span>
                          </div>
                        ))}
                      </div>
