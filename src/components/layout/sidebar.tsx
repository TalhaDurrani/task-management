...
export function Sidebar() {
  ...
  const { selectedWorkspace, workspaces, currentUserRole, setSelectedWorkspace } = useWorkspace()
  
  // Compute project count from projects array
  const projectCount = projects.length
  
  // Check if user is admin (either workspace admin or system admin)
  const isWorkspaceAdmin = currentUserRole === 'ADMIN'
  const isSystemAdmin = currentUser?.role === 'ADMIN'
  const hasAdminAccess = isWorkspaceAdmin || isSystemAdmin
  ...
  // Memoize loadProjects to prevent infinite loops
  const loadProjects = useCallback(async () => {
    try {
      // Only load projects if a workspace is selected
      if (!selectedWorkspace) {
        setProjects([])
        return
      }
      
      const url = `/api/projects?workspaceId=${selectedWorkspace.id}`
  ...
  // Memoize loadUserTasks to prevent infinite loops
  const loadUserTasks = useCallback(async (userId: string) => {
    try {
      // Only count tasks from selected workspace
      if (!selectedWorkspace) {
        setUserTaskCount(0)
        return
      }

      // Load projects for selected workspace first
      const projectsResponse = await fetch(`/api/projects?workspaceId=${selectedWorkspace.id}`)
      if (!projectsResponse.ok) {
        console.error('Failed to load projects for task count')
        setUserTaskCount(0)
        return
      }
      ...
      // Count tasks from each project in the selected workspace
  ...
  // Refresh projects when selected workspace changes
  useEffect(() => {
    if (selectedWorkspace) {
      loadProjects()
    }
  ...
  // Refresh task count when workspace changes
  useEffect(() => {
    if (selectedWorkspace && currentUser) {
      loadUserTasks(currentUser.id)
    }
  }, [selectedWorkspace?.id, currentUser?.id, loadUserTasks])

  // Refresh projects when pathname changes (for project creation)
  useEffect(() => {
  ...
  const loadData = async () => {
    try {
      ...
      if (result.success && result.user) {
        ...
        // Load user tasks and inbox count (projects will be loaded by useEffect when workspace is ready)
        await Promise.all([
  ...
  return (
    <div className={cn(
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-6">
        ...
        {/* Admin Section - Only show for workspace admins or system admins */}
        {hasAdminAccess && (
