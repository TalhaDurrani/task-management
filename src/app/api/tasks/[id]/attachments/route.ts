...
export async function POST(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const fileSize = parseInt((formData.get('fileSize') as string) || '0') || file?.size || 0
    ...
    const attachment = await prisma.attachment.create({
      data: {
        taskId: params.id,
        fileName,
        filePath, // In production, this would be the S3/cloud URL
        fileSize,
        mimeType,
        uploadedBy: user.id
      },
    ...
    console.log('✅ File uploaded successfully:', { fileName, fileSize, mimeType })
